...
import { Card, Button, Badge, Spinner, Modal, Input } from '../../components/common';
...
const JobDetail = () => {
    ...
    const handleSubmitApplication = async () => {
        ...
        try {
            ...
            setApplicationData({ name: user?.username || '', phone: '', cv_link: '' });
        } catch (error: any) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Card className="p-8">
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                            <div className="flex-1">
                                ...
                                <p className="text-xl font-medium text-primary">{job.company_name}</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-8 border-y border-gray-100 dark:border-gray-800 mb-8">
                            <div>
                                <p className="text-xs font-semibold text-text-muted uppercase mb-1">Location</p>
                                <p className="text-sm font-bold text-text-main">{job.location}</p>
                            </div>
                            <div>
                                <p className="text-xs font-semibold text-text-muted uppercase mb-1">Salary</p>
                                <p className="text-sm font-bold text-text-main">{job.salary_range || 'Competitive'}</p>
                            </div>
                            <div>
                                <p className="text-xs font-semibold text-text-muted uppercase mb-1">Type</p>
                                <p className="text-sm font-bold text-text-main capitalize">{job.job_type.replace('-', ' ')}</p>
                            </div>
                            <div>
                                <p className="text-xs font-semibold text-text-muted uppercase mb-1">Deadline</p>
                                <p className="text-sm font-bold text-text-main">{new Date(job.deadline).toLocaleDateString()}</p>
                            </div>
                        </div>

                        <div className="space-y-6">
                            <div>
                                <h3 className="text-xl font-bold text-text-main mb-4">Job Description</h3>
                ...
                <aside className="lg:col-span-1">
                    <div className="sticky top-8 space-y-6">
                        <Card className="p-6">
                            <h3 className="text-lg font-bold text-text-main mb-6">About this role</h3>
                            ...
                            <Button variant="primary" fullWidth size="lg" onClick={handleApplyClick}>
                                Apply for this job
                            </Button>
                        </Card>

                        <Card className="p-6">
                            <h3 className="text-lg font-bold text-text-main mb-4">Share this job</h3>
                            <div className="flex gap-2">
                                <Button variant="ghost" className="flex-1">LinkedIn</Button>
                                <Button variant="ghost" className="flex-1">Twitter</Button>
                            </div>
                        </Card>
                    </div>
                </aside>
            ...
            >
                <div className="space-y-6">
                    ...
                    <Input
                        ...
                        onChange={(e) => setApplicationData({ ...applicationData, cv_link: e.target.value })}
                        required
                    />

                    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 text-sm text-text-muted">
                        <p className="font-semibold mb-2">📌 Tips for a successful application:</p>
                        <ul className="list-disc list-inside space-y-1">
                            <li>Upload your CV to Google Drive or Dropbox and share the link</li>
